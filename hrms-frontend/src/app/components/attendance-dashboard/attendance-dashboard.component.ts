...
export class AttendanceDashboardComponent implements OnInit {
    ...
    manualDate: string = '';
    manualTime: string = '';
    showManualCheckoutModal: boolean = false;
    ...
    openManualCheckoutModal(): void {
        this.manualDate = this.getTodayDate();
        this.manualTime = '18:00'; // Default checkout time
        this.showManualCheckoutModal = true;
    }

    closeManualCheckoutModal(): void {
        this.showManualCheckoutModal = false;
        this.manualEmployeeId = '';
    }

    submitManualCheckOut(): void {
        if (!this.manualEmployeeId || !this.manualDate || !this.manualTime) {
            this.showError('All fields are required');
            return;
        }

        this.isLoadingHistory = true;
        this.attendanceService.manualCheckOut(this.manualEmployeeId, this.manualDate, this.manualTime).subscribe({
            next: (response) => {
                this.showSuccess('Manual checkout successful!');
                this.closeManualCheckoutModal();
                this.onRefresh();
                this.isLoadingHistory = false;
            },
            error: (err) => {
                this.showError('Manual checkout failed: ' + (err.error?.message || err.message));
                this.isLoadingHistory = false;
            }
        });
    }
